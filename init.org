...
Counsel provides a UI for Ivy (as its backend) so it looks nicer.
...
Sorts/filters results in Ivy buffer based on most recently and most frequently used.
...
Make sure to run =M-x all-the-icons-install-fonts= after installing for everything to work correctly. After installing you may need to manually add the installed fonts to the system's font library.

If after installing the fonts the icons still don't show up correctly in Emacs, you may need to also install [[eww:https://www.nerdfonts.com/][nerd-fonts]].
...
A package to more easily navigate a project directory.
...
Provide Counsel UI for Projectile.
...
An alternative to the classic =grep= program for searching for content within files. =rg= is much faster than =grep= and has better support for unicode files. Projectile has built-in support to use =rg= in place of =grep= if it is installed.

In addition ro running the snippet below to install the =rg= package in Emacs, you must also have the [[eww:https://github.com/BurntSushi/ripgrep][ripgrep]] backend installed locally.
...
(Alternative to =rg= is =ag=. They just use different backends, =ag= uses [[eww:https://github.com/ggreer/the_silver_searcher]["The Silver Searcher"]].)
...
Also adding the following package to ensure the PATH used by Eshell is the same as the one used by my native terminal. This ensures that the commands that work in my native term also work in Eshell.
#+begin_src emacs-lisp
  (use-package
...
Manages language servers. Also add =lsp-ui= for a better UI when interacting with language servers.
...
For the *Python language server* we use =pyright= (made by Microsoft). This makes the Python coding experience almost exactly like VSCode. For this to work, you need to have Node JS (and =npm=) installed on your device and then install =pyright= by running,

#+begin_src bash
  npm install pyright
#+end_src

Then the language server should work as expected (together with Projectile, when you open a Python project the LSP will automatically become active).
...
Here's a quick function to open my init.org file at any time from anywhere (useful if I want to make a quick change without going through and typing the whole file path from root).
